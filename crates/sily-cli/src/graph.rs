...
//!
//! Layout: each session is a node (`●`). Beneath it, as a tree, hang its commits
//! (`◆`) and any sessions branched from it (`○`), recursively. Colors degrade
//! gracefully — `owo-colors` strips them when output isn't a TTY or `NO_COLOR`
//! is set, so piped output stays clean.
...
/// Render the full session/branch graph for `sily list`.
pub fn render_list(
    sessions: &[SessionRef],
    commits: &[Commit],
    ...
    // commits grouped by the session they point into
    ...
    // branch children grouped by parent (only when both ends are present)
    ...
    // roots: sessions that aren't a known branch-child, newest first
    let mut roots: Vec<&SessionRef> = sessions
        .iter()
        .filter(|s| !is_child.get(s.id.as_str()).copied().unwrap_or(false))
        .collect();
    ...
    for root in roots {
        out.push_str(&session_line(root, "●"));
        print_children(&ctx, &root.id, "", &mut out);
        out.push('\n');
    }
    out
}
...
fn print_children(ctx: &Ctx, session_id: &str, prefix: &str, out: &mut String) {
    ...
    if let Some(children) = children {
        for b in children {
            ...
            let child = ctx.by_id.get(b.session_id.as_str());
            out.push_str(&format!(
                "{}{} {}\n",
                prefix.if_supports_color(Stdout, |t| t.dimmed()),
                connector.if_supports_color(Stdout, |t| t.dimmed()),
                branch_label(child, b),
            ));
...
fn session_line(s: &SessionRef, marker: &str) -> String {
    format!(
        "{} {}  {}   {}\n",
        marker.if_supports_color(Stdout, |t| t.bright_yellow()),
